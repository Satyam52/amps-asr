...
class UnitYDataLoader:
    ...
    def _get_tokenized_source_text(self, sample: LangPairSample) -> Tensor:
        """Expected sequence is [<eos>, <lang_tok> , ..text tokens.., <eos>]"""
        source_lang = sample.source.lang
        if source_lang not in self.text_encoders_per_lang:
            self.text_encoders_per_lang[
                source_lang
            ] = self.text_tokenizer.create_encoder(lang=source_lang, mode="target")
        tokens = self.text_encoders_per_lang[source_lang](sample.source.text)
    ...
    def _get_tokenized_paraphrased_target_text(self, sample: LangPairSample) -> Tensor:
        """Expected sequence is [<eos>, <lang_tok> , ..text tokens.., <eos>]"""
        target_lang = sample.target.lang
        if target_lang not in self.text_encoders_per_lang:
            self.text_encoders_per_lang[
        ...
        tokens = self.text_encoders_per_lang[target_lang](sample.target.paraphrase)
        eos_idx = self.text_tokenizer.vocab_info.eos_idx
        tokens = torch.concat([tokens, torch.LongTensor([eos_idx])])
        return tokens
    ...
    def _prepare_batch(self, raw_samples: List[Dict[str, Any]]) -> MultimodalSeqsBatch:
        ...
        speech_src_tokens = None
        speech_src_lengths = None
        
        # logger.debug(f"Preparing batch of {len(samples)} samples")
        ...
        if self.mode == "SPEECH_TO_TEXT":
            ...
            speech_src_tokens = self._batch_tensors(
                src_tokens_list, pad_value=self.batching_config.fbank_feats_pad_idx
            ).to(self.batching_config.float_dtype)
            speech_src_lengths = torch.LongTensor(
                [src_tokens.shape[0] for src_tokens in src_tokens_list]
            )
        elif self.mode == "TEXT_TO_TEXT":
        # input text
            text_pad_idx = self.text_tokenizer.vocab_info.pad_idx
            src_tokens_list = [self._get_tokenized_source_text(sample) for sample in samples]        
            text_src_tokens = self._batch_tensors(
                [src_tokens[1:] for src_tokens in src_tokens_list], pad_value=text_pad_idx
            )
            text_src_lengths = torch.LongTensor(
                [src_tokens.shape[0] - 1 for src_tokens in src_tokens_list]
            )
        elif self.mode == "SPEECH_TO_PARA":
            src_tokens_list = [self._get_source_fbank(sample) for sample in samples]
            speech_src_tokens = self._batch_tensors(
                src_tokens_list, pad_value=self.batching_config.fbank_feats_pad_idx
            ).to(self.batching_config.float_dtype)
        ...
        para_tokens_list = [
            self._get_tokenized_paraphrased_target_text(sample) for sample in samples
        ]
        prev_para_outputs_tokens = self._batch_tensors(
            [tokens[:-1] for tokens in para_tokens_list], pad_value=text_pad_idx
        )
        para_target_tokens = self._batch_tensors(
            [tokens[1:] for tokens in para_tokens_list], pad_value=text_pad_idx
        )
        para_tokens_lengths = torch.LongTensor(
            [tokens.shape[0] - 1 for tokens in para_tokens_list]
        )
        ...
        return MultimodalSeqsBatch(
            speech_to_text=SeqsBatch(
                src_tokens=speech_src_tokens,
                src_lengths=speech_src_lengths,
                target_tokens=target_tokens,
                prev_output_tokens=prev_outputs_tokens,
                target_lengths=tokens_lengths,
            ),
            text_to_text=SeqsBatch(
                src_tokens=text_src_tokens,
                src_lengths=text_src_lengths,
                target_tokens=para_target_tokens,
                prev_output_tokens=prev_para_outputs_tokens,
                target_lengths=para_tokens_lengths,
            ),
            text_to_units=SeqsBatch(
                src_tokens=None,
                src_lengths=None,
