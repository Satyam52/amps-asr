...
class FinetuneMode(Enum):
    ...
    SPEECH_TO_PARA = "SPEECH_TO_PARA"


s2t_loss_ratio: int = 0
t2t_loss_ratio: int = 0
threshold_loss: int = 4 # -1 for no threshold
...
class FinetuneParams:
    ...
    t2t_loss_ratio: int = -1
    s2t_loss_ratio: int = -1
    threshold_loss: int = -1
...
class UnitYFinetuneWrapper(nn.Module):
    ...
    def __init__(self, model: UnitYModel, mode: FinetuneMode, device: Device):
        ...
        self.is_paraphrase: bool = mode == FinetuneMode.SPEECH_TO_PARA
        self.is_t2t: bool = mode == FinetuneMode.TEXT_TO_TEXT
    ...
    def forward(
        ...
        if not self.is_t2t and not self.is_paraphrase:
            ...
            return (text_logits, unit_logits, None)
        elif self.is_t2t:
            assert batch.text_to_text.src_tokens is not None
            seqs = batch.text_to_text.src_tokens.to(self.device)
            seq_lens = batch.text_to_text.src_lengths.to(self.device)
            text_encoder_out, text_encoder_padding_mask = self.model.encode_text(
                seqs=seqs, padding_mask=PaddingMask(seq_lens, seqs.size(1))
            )
            assert batch.text_to_text.prev_output_tokens is not None
            seqs = batch.text_to_text.prev_output_tokens.to(self.device)
            seq_lens = batch.text_to_text.target_lengths.to(self.device)
            text_decoder_out, text_decoder_padding_mask = self.model.decode(
                seqs=seqs,
                padding_mask=PaddingMask(seq_lens, seqs.size(1)),
                encoder_output=text_encoder_out,
                encoder_padding_mask=text_encoder_padding_mask,
            )
            text_logits = self.model.final_proj(text_decoder_out)
            return (text_logits, None, None)
        elif self.is_paraphrase:
            speech_text_logits = None
            text_text_logits = None
            
            if s2t_loss_ratio <= 0 and t2t_loss_ratio <= 0:
                raise NotImplementedError("s2t and t2t loss ratio both can not be zero")
            
            if s2t_loss_ratio > 0:
                ## pass speech to get text
                assert batch.speech_to_text.src_tokens is not None
                seqs = batch.speech_to_text.src_tokens.to(self.device)
                seq_lens = batch.speech_to_text.src_lengths.to(self.device)
                speech_encoder_out, speech_encoder_padding_mask = self.model.encode_speech(
                ...
                speech_text_logits = self.model.final_proj(text_decoder_out)
            
            if t2t_loss_ratio > 0:
                # Now pass text to get text
                assert batch.text_to_text.src_tokens is not None
                text_seqs = batch.text_to_text.src_tokens.to(self.device)
                text_seq_lens = batch.text_to_text.src_lengths.to(self.device)
                text_encoder_out, text_encoder_padding_mask = self.model.encode_text(
                    seqs=text_seqs, padding_mask=PaddingMask(text_seq_lens, text_seqs.size(1))
                )
                assert batch.text_to_text.prev_output_tokens is not None
                text_seqs = batch.text_to_text.prev_output_tokens.to(self.device)
                text_seq_lens = batch.text_to_text.target_lengths.to(self.device)
                text_decoder_out, text_decoder_padding_mask = self.model.decode(
                    seqs=text_seqs,
                    padding_mask=PaddingMask(text_seq_lens, text_seqs.size(1)),
                    encoder_output=text_encoder_out,
                    encoder_padding_mask=text_encoder_padding_mask,
                )
                text_text_logits = self.model.final_proj(text_decoder_out)
                
            return (speech_text_logits, None, text_text_logits)
...
class CalcLoss:
    ...
    def __init__(
        ...
        self.is_t2t: bool = is_t2t
        self.is_paraphrase: bool = is_paraphrase
        self.t2t_vocab_info = t2t_vocab_info
        self.count = 0
    ...
    def __call__(
        ...
        paraphrase_logits: Optional[torch.Tensor],
        training: bool = False,
    ) -> torch.Tensor:
        if not self.is_t2t and not self.is_paraphrase:
            ...
            assert batch.text_to_text.target_lengths is not None
            t2t_numel = torch.sum(batch.text_to_text.target_lengths).to(
                text_logits.device
            )
            t2t_loss = SequenceModelOutput(
                logits=text_logits, vocab_info=self.t2t_vocab_info
            ).compute_loss(
                targets=batch.text_to_text.target_tokens.to(text_logits.device),
                ignore_prefix_size=1,
                label_smoothing=self.label_smoothing,
            )
            return t2t_loss / t2t_numel
        elif self.is_paraphrase:
            s2t_loss, t2t_loss, s2t_numel, t2t_numel = 0, 0, 1, 1
            ...
            if t2t_loss_ratio > 0:
                assert batch.text_to_text.target_lengths is not None
                t2t_numel = torch.sum(batch.text_to_text.target_lengths).to(
                    paraphrase_logits.device
                )
                t2t_loss = SequenceModelOutput(
                    logits=paraphrase_logits, vocab_info=self.t2t_vocab_info
                ).compute_loss(
                    targets=batch.text_to_text.target_tokens.to(paraphrase_logits.device),
                    ignore_prefix_size=1,
                    label_smoothing=self.label_smoothing,
                )
                
            self.count += 1
            s2t_final_loss = s2t_loss / s2t_numel
            t2t_final_loss = t2t_loss / t2t_numel
            
            
            final_loss = 0
            if threshold_loss < 0:
                final_loss = (s2t_loss_ratio/(s2t_loss_ratio + t2t_loss_ratio))*(s2t_loss / s2t_numel) + (t2t_loss_ratio/(s2t_loss_ratio + t2t_loss_ratio))*(t2t_loss / t2t_numel)
        
            if s2t_final_loss > threshold_loss:
                final_loss = (s2t_loss_ratio/(s2t_loss_ratio + t2t_loss_ratio))*(s2t_loss / s2t_numel) + (t2t_loss_ratio/(s2t_loss_ratio + t2t_loss_ratio))*(t2t_loss / t2t_numel)

            else:
                final_loss = s2t_final_loss
            
            
            if self.count % 100 == 0:
                logger.info(f"Threshold: {threshold_loss:.4f}, S2T loss: {s2t_final_loss:.4f}, T2T loss: {t2t_final_loss:.4f}, Final loss: {final_loss:.4f}")
...
class UnitYFinetune:
    def __init__(
        ...
        ## SET GLOBAL loss ratio ##
        global s2t_loss_ratio
        global t2t_loss_ratio
        global threshold_loss
        s2t_loss_ratio = self.params.s2t_loss_ratio
        t2t_loss_ratio = self.params.t2t_loss_ratio
        threshold_loss = self.params.threshold_loss
        logger.info(f"Setting: S2T loss ratio: {s2t_loss_ratio}, T2T loss ratio: {t2t_loss_ratio}, Threshold loss: {threshold_loss}")
        ## SET GLOBAL loss ratio ##
        ...
        self.calc_loss = CalcLoss(
            ...
            t2t_vocab_info=model.target_vocab_info,
            is_t2t=self.params.finetune_mode == FinetuneMode.TEXT_TO_TEXT,
            is_paraphrase=self.params.finetune_mode == FinetuneMode.SPEECH_TO_PARA
        )
    ...
    def _eval_model(self) -> None:
        ...
        with torch.no_grad():
            for batch in self.eval_data_loader.get_dataloader():
                if self.params.finetune_mode == FinetuneMode.TEXT_TO_TEXT:
                    assert batch.text_to_text.src_tokens is not None
                elif self.params.finetune_mode == FinetuneMode.SPEECH_TO_TEXT:
                    assert batch.speech_to_text.src_tokens is not None
                elif self.params.finetune_mode == FinetuneMode.SPEECH_TO_PARA:
                    assert batch.speech_to_text.src_tokens is not None
                    assert batch.text_to_text.src_tokens is not None
                else:
                    raise ValueError(f"Unsupported mode: {self.params.finetune_mode}")
                
                op = self.model(batch)
                loss = self.calc_loss(batch, *self.model(batch), training=False)
                if loss.isnan():
    ...
    def _train_step(self, batch: dataloader.MultimodalSeqsBatch) -> None:
        ...
        tokens, units, para = self.model(batch)
        loss = self.calc_loss(batch, tokens, units, para, training=True)
