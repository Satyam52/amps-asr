...
@final
class MLMAttentionMask(AttentionMask):
    """Represents a causal attention mask.
    ...
    *Shape:* :math:`(S,S_{kv})`, where :math:`S` is the sequence length and
    ...
    @finaloverride
    def _do_materialize(self) -> Tensor:
        return _create_mlm_attention_mask(
            self.seq_len, self.key_len, self.attn_window_len, self.device, self.dtype
        )


class MLMAttentionMaskFactory:
    """Constructs instances of :class:`CausalAttentionMask`."""
    ...
    def __repr__(self) -> str:
        if self.attn_window_len is None:
            return "MLMAttentionMaskFactory()"

        return f"MLMAttentionMaskFactory(attn_window_len={self.attn_window_len})"


def _create_mlm_attention_mask(
    seq_len: int,
    key_len: int,
    attn_window_len: Optional[int],
    device: Optional[Device],
    ...
    # Mask 15% of the keys randomly, same for all sequences
    num_masked = int(0.15 * key_len)
    m_keys = torch.randint(0, key_len, (num_masked,))
    ...
    # print(m_keys, num_masked)
    # print(f"% of keys masked: {num_masked/key_len}*")
    # print(mask)
    # print(mask.shape)
    # pudb.set_trace()
    # raise ValueError("Stop here")
    
    # # Create attention window if specified
    # if attn_window_len is not None:
    #     window_mask = torch.ones((seq_len, key_len), device=device, dtype=dt)
    #     window_mask.triu_(diagonal=attn_window_len)
    #     window_mask.tril_(diagonal=-attn_window_len)
    #     mask[window_mask.bool()] = float('-inf')
