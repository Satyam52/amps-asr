...
class ConformerBlock(TransformerEncoderLayer):
    ...
    speech_language_adaptor_ffn: FeedForwardNetwork
    ...
    def __init__(
        ...
        *,
        ...
        self.speech_language_adaptor_ffn = speech_language_adaptor_ffn
        self.speech_language_adaptor_ffn_layer_norm = layer_norm_factory(model_dim, device=device, dtype=dtype)
    ...
    def forward(
        ...
        seqs = self._adapter_ffn(seqs)

        seqs = self.layer_norm(seqs)

        return seqs, padding_mask
    
    def _adapter_ffn(self, seqs: Tensor) -> Tensor:
        residual = seqs

        seqs = self.speech_language_adaptor_ffn_layer_norm(seqs)

        seqs = self.speech_language_adaptor_ffn(seqs) * 0.5
