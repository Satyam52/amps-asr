...
class StandardTransformerDecoderLayer(TransformerDecoderLayer):
    ...
    def __init__(
        ...
        *,
        ...
        text_language_adaptor_ffn_norm1 = layer_norm_factory(model_dim, device=device, dtype=dtype)

        if norm_order != TransformerNormOrder.POST:
            self.ffn_layer_norm = ffn_layer_norm
            self.text_language_adaptor_ffn_norm1 = text_language_adaptor_ffn_norm1

        self.ffn = ffn
        self.text_language_adaptor_ffn = text_language_adaptor_ffn
    ...
    def forward(
        ...
        *,
        ...
        seqs = self._adapter_ffn(residual, seqs)
        

        return seqs, padding_mask

    
    def _adapter_ffn(self, residual:Tensor, seqs: Tensor) -> Tensor:

        if self.norm_order != TransformerNormOrder.POST:
            seqs = self.text_language_adaptor_ffn_norm1(seqs)

        seqs = self.text_language_adaptor_ffn(seqs)
